...
rb_root* rbt_init(void);
static void rbt_handleReorient(rb_root* T, rbnode* x, int k);
rb_root* rbt_insert(rb_root* &T, int k);
rb_root* rbt_delete(rb_root* &T, int k);

void rbt_transplant(rb_root* T, rbnode* u, rbnode* v);

static void rbt_left_rotate( rb_root* T, rbnode* x);
static void rbt_right_rotate( rb_root* T, rbnode* x);

void rbt_inPrint(const rb_root* T, rbnode* t);
void rbt_prePrint(const rbnode * T, rbnode* t);
void rbt_print(const rb_root* T);
...
*
*/ 
void rbt_handleReorient(rb_root* T, rbnode* x, int k){
...
rb_root* rbt_delete(rb_root* &T, int k){
    ...
    if(toDelete->left != T->nil && toDelete->right != T->nil){
        rbnode* alternative = rbt_findMin(T, toDelete->right);
...
*@param[in] x 要进行旋转的结点
*/
void rbt_left_rotate( rb_root* T, rbnode* x){
    rbnode* y = x->right;
...
*@param[in] 要进行旋转的结点
*/
void rbt_right_rotate( rb_root* T, rbnode* x){
    rbnode * y = x->left;
    x->left = y->right;
...
void rbt_prePrint(const rb_root* T, rbnode* t){
    if(T->nil == t)return ;
    if(t->color == RED)
        printf("%3dR",t->key);
    else
